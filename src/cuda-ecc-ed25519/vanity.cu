...
#undef RND
		...
		// Confronto dei suffissi
		for (int i = 0; i < sizeof(suffixes) / sizeof(suffixes[0]); ++i)
		{

			// Calcola la lunghezza effettiva della chiave (escludendo il terminatore nullo)
			size_t key_length = strlen(key);

			// Ottieni la lunghezza del suffisso corrente
			int suffix_length = suffix_letter_counts[i];

			// Verifica che la chiave sia abbastanza lunga per contenere il suffisso
			if (key_length < suffix_length)
			{
				continue; // Salta questo suffisso se la chiave è troppo corta
			}

			// Calcola l'indice di partenza per il confronto del suffisso
			int start_index = key_length - suffix_length;

			bool match = true; // Flag per indicare se il suffisso corrisponde

			for (int j = 0; j < suffix_length; ++j)
			{

				// Confronto del suffisso
				if (!(suffixes[i][j] == '?') && !(suffixes[i][j] == key[start_index + j]))
				{
					match = false;
					break; // Esce dal ciclo interno se non c'è corrispondenza
				}
			}

			// Se il suffisso corrisponde completamente
			if (match)
			{
				atomicAdd(keys_found, 1);

				// Stampa delle informazioni della chiave corrispondente
				...
				*/
				...
				break; // Esce dal ciclo dei suffissi una volta trovata una corrispondenza
			}
		}
